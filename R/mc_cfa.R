#' Monte Carlo Sample Size Estimation for CFA (A Posteriori)
#'
...
#' @param model A character string specifying the lavaan model syntax
...
#'
#' # Run analysis
#' result <- mc_cfa(data = mydata, model = model)
#'
#' # View results
#' print(result)
#'
#' # Save plot
#' ggsave("sample_size_plot.png", result$plot, width = 12, height = 9)
#' }
#'
#' @export
mc_cfa <- function(data,
                   model,
  ...
  # Extract item names from model
  item_names <- unique(unlist(regmatches(model, gregexpr("\\b[a-zA-Z_][a-zA-Z0-9_]*\\b", model))))
  item_names <- item_names[!grepl("^F[0-9]+$", item_names)]  # Remove factor names
  item_names <- item_names[item_names %in% names(data)]

  if (length(item_names) == 0) {
    stop("No items found in data matching the model specification")
  }
  ...
  if (verbose) {
    cat("\n")
    cat("================================================================\n")
    cat("   MONTE CARLO SAMPLE SIZE ANALYSIS FOR CFA (A POSTERIORI)\n")
    cat("================================================================\n\n")
  ...
  # Create plot
  subtitle <- sprintf("Criterion: %s | Replications: %d | %s correlation",
                      criterion, reps, cor_type)

#' Monte Carlo Sample Size Estimation for CFA (A Priori)
#'
#' Estimates optimal sample size for Confirmatory Factor Analysis using
...
#' @param misspecification Level of model misspecification:
#'   "none", "minor", "moderate", or "severe" (default: "moderate")
...
#' @examples
#' \dontrun{
#' # Simple example: 4 factors, 4 items each, Likert 5-point scale
#' result <- mc_cfa_apriori(
#'   n_factors = 4,
#'   items_per_factor = 4,
#'   loadings = 0.70,
#'   n_categories = 5
...
#'
#' # Save plot
#' ggsave("apriori_plot.png", result$plot, width = 12, height = 9)
#'
#' # Different items per factor
#' result2 <- mc_cfa_apriori(
#'   n_factors = 3,
#'   items_per_factor = c(5, 4, 4),
#'   loadings = 0.65,
#'   misspecification = "minor"
#' )
#' }
#'
#' @export
mc_cfa_apriori <- function(n_factors,
                            items_per_factor,
                            loadings = 0.70,
                            factor_cors = 0.30,
                            n_categories = 5,
  ...
  if (length(items_per_factor) == 1) {
    items_per_factor <- rep(items_per_factor, n_factors)
  }
  n_items <- sum(items_per_factor)

  # Get thresholds and misspecification params
  thresholds <- .get_thresholds(criterion)
  misspec_params <- .config_misspecification(misspecification)

  # Create theoretical model components
  loading_matrix <- .create_loading_matrix(n_factors, items_per_factor, loadings,
                                            cross_loading_mag = misspec_params$cross_loadings)
  phi_matrix <- .create_phi_matrix(n_factors, factor_cors)
  residual_matrix <- .create_residual_matrix(n_items,
                                              residual_cor_mag = misspec_params$residual_cors,
                                              prop_correlated = misspec_params$prop_residuals)
  sigma_pop <- .generate_sigma(loading_matrix, phi_matrix, residual_matrix)

  # Create lavaan model syntax (simple model without cross-loadings)
  model_syntax <- .create_lavaan_syntax(n_factors, items_per_factor)

  if (verbose) {
    cat("\n")
    cat("================================================================\n")
    cat("   MONTE CARLO SAMPLE SIZE ANALYSIS FOR CFA (A PRIORI)\n")
    cat("================================================================\n\n")
    ...
    cat(sprintf("  Level: %s\n", misspecification))
    cat(sprintf("  Cross-loadings: %.2f\n", misspec_params$cross_loadings))
    cat(sprintf("  Residual correlations: %.2f (%.0f%% of pairs)\n",
                misspec_params$residual_cors, misspec_params$prop_residuals * 100))
    cat("\n  NOTE: The FITTED model is simple (no cross-loadings),\n")
    cat("        but DATA is generated with realistic misspecification.\n\n")
  ...
  # Create plot
  subtitle <- sprintf("Misspecification: %s | Criterion: %s | %d replications",
                      misspecification, criterion, reps)
  plot_obj <- .create_full_plot(results_df, n_recommended, thresholds,
